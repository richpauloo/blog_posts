...
This blog post is my attempt to process and expand on an excellent talk delivered by Andrew Bray at rstudio::conf2018. [The slides (PDF) from Andrew's presentation can be found at this github link](https://github.com/andrewpbray/talks/blob/master/2018/infer-rstudioconf-2018.pdf).  

From Infer's [homepage](http://infer.netlify.com/):  
...
Infer is supposed to play well within the tidyverse, so we load that too, along with some data from the [General Social Survey](http://gss.norc.org/). In this dataset, we see the diversity of information collected. 
```{r, warning=FALSE, message=FALSE}
library(tidyverse)
...
![*A slide from Andrew Bray's presentation.*](infer_flow.png)
...
* `visualize()` automatically plots permutated with ggplot, making it easy to add geoms onto it  
...
If we wanted to get a p-value from this programatic appraoch, we can calculate the area uner the curve to the right of the observed statistic.  
```{r}
gss %>% 
  specify(NASA ~ party)  %>% 
  hypothesize(null = "independence") %>% 
...
One of the key features of `infer`, is that the functions therein are reusable. To illustrate, let's consider our data. Let's say we wanted to look at the differences between men and women in their reported level of happiness. We could observe this with a simple bar plot.  
...
knitr::include_graphics('TGIF.gif')
```


`Infer` helps us assess our uncertainty in this statistic by calculating a confidence interval, which adds two standard errors to the point estimate.  
...
```{r, out.width = "1000px", echo=FALSE}
knitr::include_graphics('carey_dancing.gif')
```
*The joy of using infer for tidy hypothesis testing*   


## Conclusion  
Two things stand out to me about `infer`. One if that is makes me think through the steps of what I'm doing, and the verbs in `infer` are reusable. Two, I save time because everything happens in a tidy way that plays well with `dplyr`. This makes it easy to hypothesis test, mutate and summarise new variables, pipe results into a `ggplot`, and overall spend less time working with lists and other data structures that I wrestle with in base R.    

I look forward to seeing what's down the line from `infer`. It seems like a powerful and concise way to perform statistical inference and hypothesis test. Thanks to Andrew Bray for the excellent talk, and the developers of `infer` for working on tidy and expressive statistical inference.  
